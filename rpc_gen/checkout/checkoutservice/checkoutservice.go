...
func checkoutHandler(ctx context.Context, handler interface{}, arg, result interface{}) error {
	...
	case *streaming.Args:
		st := s.Stream
		req := new(checkout.CheckoutRequest)
...
func (p *CheckoutArgs) Unmarshal(in []byte) error {
	msg := new(checkout.CheckoutRequest)
...
var CheckoutArgs_Req_DEFAULT *checkout.CheckoutRequest

func (p *CheckoutArgs) GetReq() *checkout.CheckoutRequest {
	if !p.IsSetReq() {
...
var CheckoutResult_Success_DEFAULT *checkout.CheckoutResponse

func (p *CheckoutResult) FastRead(buf []byte, _type int8, number int32) (n int, err error) {
	if !p.IsSetSuccess() {
		p.Success = new(checkout.CheckoutResponse)
	}
...
func (p *CheckoutResult) Unmarshal(in []byte) error {
	msg := new(checkout.CheckoutResponse)
	if err := proto.Unmarshal(in, msg); err != nil {
		return err
	}
	p.Success = msg
...
func (p *CheckoutResult) GetSuccess() *checkout.CheckoutResponse {
	if !p.IsSetSuccess() {
...
func (p *CheckoutResult) SetSuccess(x interface{}) {
	p.Success = x.(*checkout.CheckoutResponse)
}
...
func (p *kClient) Checkout(ctx context.Context, Req *checkout.CheckoutRequest) (r *checkout.CheckoutResponse, err error) {
