...
func TestCreateProduct(t *testing.T) {
	h := server.Default()
	h.POST("/product/create", CreateProduct)
	path := "/product/create"                                 // todo: you can customize query
	body := &ut.Body{Body: bytes.NewBufferString(""), Len: 1} // todo: you can customize body
	header := ut.Header{}                                     // todo: you can customize header
...
func TestAssociateProductWithCategory(t *testing.T) {
	h := server.Default()
	h.POST("/product/associateProdWithCategory", AssociateProductWithCategory)
	path := "/product/associateProdWithCategory"              // todo: you can customize query
	body := &ut.Body{Body: bytes.NewBufferString(""), Len: 1} // todo: you can customize body
	header := ut.Header{}                                     // todo: you can customize header
	w := ut.PerformRequest(h.Engine, "POST", path, body, header)
	resp := w.Result()
